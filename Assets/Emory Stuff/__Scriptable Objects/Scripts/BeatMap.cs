...
using FMODUnity;
[CreateAssetMenu(/* fileName = "BeatMap", */ menuName = "Scriptable Objects/Rhythm/BeatMap")]
public class BeatMap : ScriptableObject
{
    ...
    private void onBeats(int beatNum, int id)
    {
        ...
        {
            if (currentTime < startTime + timeLoop)
                timeLoop = 0;
            ...
            if (beatCounter.getCurrentBeatTime(beatNum) < endTime || endTime <= startTime)
            {
                ...
                foreach (var beat in beats)
                {
                   ...
                   (startTime + timeLoop) == beat.beatNum)
                        if (beat.type != BEAT_TYPE.REPEAT)
                            onNewNote.Invoke(beat, id);
                        else
                            timeLoop = (beatCounter.getCurrentBeatTime(beatNum) - startTime);
                    //timeLoop = (beatCounter.getCurrentBeatTime(beatNum) - startTime);

                    finalbeat = beat.beatNum > finalbeat && beat.type == BEAT_TYPE.REPEAT ? beat.beatNum : finalbeat;
                }
